...
@section('content')
    ...
    <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
            <h1 class="text-2xl font-extrabold tracking-tight text-slate-900">Empresas</h1>
            <p class="mt-1 text-sm text-slate-500">{{ count($companies) }} empresa(s) cadastrada(s).</p>
        </div>
        <a href="/management/company/create"
    ...
    <div class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        @if (count($companies))
            <table class="w-full text-left text-sm">
                <thead class="bg-slate-50 text-xs uppercase tracking-wide text-slate-500">
                    <tr>
                        <th scope="col" class="px-6 py-3 font-bold">Nome da empresa</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    @foreach ($companies as $company)
                        <tr class="transition hover:bg-slate-50">
                            <td class="px-6 py-4 font-semibold text-slate-900">{{ $company->companyName }}</td>
                            <td class="px-6 py-4">
                                <div class="flex items-center justify-end gap-2">
                                    <a href="/management/company/edit/{{ $company->id }}"
                                    ...
                                    <form action="/management/company/{{ $company->id }}" method="POST"
                                        onsubmit="return confirm('Excluir a empresa {{ $company->companyName }}? Esta ação não pode ser desfeita.');">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
        ...
        @else
            <div class="flex flex-col items-center justify-center px-6 py-16 text-center">
                ...
                <p class="mt-4 font-bold text-slate-900">Nenhuma empresa cadastrada</p>
                <p class="mt-1 text-sm text-slate-500">Comece adicionando sua primeira empresa.</p>
                <a href="/management/company/create"
